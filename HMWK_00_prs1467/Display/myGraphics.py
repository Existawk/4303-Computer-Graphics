...
class _frame :
  def __init__( self, world ) :
    ...
    # Here's the spot where the background color of the display
    #   window is set.  By default, it's a neutral shade of grey,
    #   #C0C0C0.
    self.canvas = tk.Canvas(
      world.rootWindow, width=1, height=1, bg='#75bb85' )
...
class _menu :
  ...
  def menu_open_callback( self ) :
    ...
    if ( len( fName ) == 0 ) :
      self.statusBar.set( "%s", "[Load was cancelled]" )
      return
...
class _statusBar( tk.Frame ) :
  ...
  def set( self, formatStr, *args ) :
    # Here's the spot where the user's NetID is prepended to the
    #   text of every message that gets displayed.
    msg = f'prs1467: {formatStr % args}'
    print( msg )
