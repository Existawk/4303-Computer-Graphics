...
Model *loadModel( char *fName )
{
  ...
  //       model->xMin.
  model->xCenter = (model->xMin + model->xMax)/2;
  // TODO: Compute model->yCenter using model->yMax and
  //       model->yMin.
  model->yCenter = (model->yMin + model->yMax)/2;
  // TODO: Compute model->zCenter using model->zMax and
  //       model->zMin.
  model->zCenter = (model->zMin + model->zMax)/2;
...
void rotateModel( Model *model, Parameters *params )
{
  ...
  //  to make this easier.)
  matrixMultMatrix(xRot,yRot,tmpA);
  matrixMultMatrix(tmpA,zRot,tmpB);
  matrixMultMatrix(fromOrigin,tmpB,tmpA);
  ...
  for ( int i=0; i<model->numPoints; i++ ) {
    ...
    memcpy(model->points[i], rotPt, sizeof(rotPt));
  }
}
